...
from src.utils.utils import extract_field_names, convert_to_struct_format, build_structs_for_local_use
...
class BinLogParser:
    ...
    def _validate_fmt_definitions(self) -> None:
        ...
        for msg_id, fmt_definition in self.fmt_definitions.items():
            try:
                struct_size = struct.calcsize(fmt_definition["struct_fmt"])
                expected = fmt_definition["message_length"] - 3
        ...
        while True:
            ...
            if not fmt_definition or "struct_obj" not in fmt_definition:
                if self.collect_warnings:
                    self.warnings.append(f"Unknown message ID at offset {position}: {message_id}")
                position += 1
                continue

            if message_filter and fmt_definition["name"] not in message_filter:
                position += fmt_definition["message_length"]
                continue
    ...
    def _decode_single_message(
        ...
        payload_end = payload_start + fmt_definition["struct_size"]

        if payload_end > end_offset:
            if self.collect_warnings:
                self.warnings.append(f"Truncated message at offset {position}")
            return None

        try:
            unpacked_values = self._unpack_values(fmt_definition, payload_start, unpack_cache)
            scaled_values = self._apply_scaling(unpacked_values, fmt_definition["ardu_format"])
    ...
    def _find_next_message(self, mapped_log: mmap.mmap, position: int, end_offset: int) -> Optional[int]:
        ...
        next_sync_position = mapped_log.find(SYNC_MARKER, position, end_offset)
    ...
    def _build_message_as_dict(self, fmt_definition: Dict, values: List) -> Dict:
        message = dict(zip(fmt_definition["field_names"], values))
    ...
    def _parse_fmt_message(self, offset: int) -> bool:
        ...
        try:
            ...
            msg_type_id = mapped_log[offset + 3]
            msg_name = mapped_log[offset + 5 : offset + 9].decode("ascii", "ignore").strip("\x00")

            if not re.match(r"^[A-Za-z0-9]+$", msg_name):
                return False

            ardu_format = mapped_log[offset + 9 : offset + 25].decode("ascii", "ignore").strip("\x00")
            field_names = extract_field_names(mapped_log[offset + 25 : offset + 89])
            struct_format = convert_to_struct_format(ardu_format, self._ardu_to_struct)
